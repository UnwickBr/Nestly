...
// action routes below, which always have a segment ([...action] matches
// one or more, so multi-segment paths like /items/comments/add work too).
export default function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (path === '/api/items/comments/add' && req.method === 'POST') return handleCommentAdd(req, res);
  res.statusCode = 404;
  res.end('Not found');
}
